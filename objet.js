...
// FAIRE APPARAITRE TOUS LES TITRES ORIGINAUX DE TOUT LE TABLEAU
// on se déplace dans les objets en mettant un point dans la méthode log
// for (let index = 0; index < results.length; index++) {
//     console.log(results[index].original_title);
// }

// DEUXIEME METHODE AVEC Map: METHODE MAP EST PLUS ADAPTEE AVEC TABLEAUX AVEC DES OBJETS DEDANS
// results.map((film) => (console.log(film.original_title)));
// ICI ON A DES PARENTHESES ET PAS D ACCOLADES CAR IL N Y A QU UNE SEULE EXECUTION




// EXERCICE: AVEC METHODE MAP SI FILM A UNE MOYENNE>=5 (VOTE AVERAGE) ALORS LE FILM EST BON SINON LE FILM EST NUL

for (const key in results) {
    if (results[key].vote_average >= 5) {
        console.log(results[key].original_title +' Ce film est bon')
    } else {
        console.log(results[key].original_title +' Ce film est nul')
    }
}

// on peut remplacer par une condition ternaire sur une ligne  et la METHODE MAP:
// on a nommé notre parametre film dans la méthode map mais on peut l'appeler comme on veut
// results.map((film) =>(
...
// EXERCICE: faire une SECTION 1 AVEC UN ID DANS LE HTML: on commence par créer une section1 avec un id dans le HTML puis on le recupère en JS avec un queryselector ou un getelementbyid
// puis on la récupère dans une constante
// const section = document.querySelector('#section1');
// console.log(section1);
// const calcul = 38*2;
// // section.innerHTML = calcul;
// section.innerHTML = `<p>Le double de mon âge est ${calcul}</p>`;
// on utilise les backticks: ça permet de concaténer avec le $ sans utiliser de +
// on peut créer directement les balises et pour donner du javascript c est ${}

// REMPLACER LA BALISE P PAR UN H2
// const section = document.querySelector('#section1');
// console.log(section1);
// section.innerHTML = results.map((film) =>
//     `<h2>Le titre original ${film.original_title}</h2>`
//     ).join('')


// POUR AFFICHER L AFFICHE DE CHAQUE FILM AVEC LE TITRE DU FILM
const section = document.querySelector('#section1');
console.log(section1);
section.innerHTML = results.map((film) =>
    `<h2>Le titre original ${film.original_title}</h2>
